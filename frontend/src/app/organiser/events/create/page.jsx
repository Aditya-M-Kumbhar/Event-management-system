...
export default function CreateEventPage() {
  ...
  const methods = useForm({
    defaultValues: {
      title: '', description: '', category: '', format: 'offline',
      startDate: '', endDate: '', totalCapacity: 100,
      'venue.name': '', 'venue.city': '', 'venue.address': '',
      onlineMeetingLink: '', status: 'draft',
              ...
              {step === 1 && (
                <div className="card p-6 space-y-5">
                  ...
                  <div>
                    ...
                    <input className="input" placeholder="e.g. NSCI Dome, Mumbai" {...register('venue.name')} />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium mb-1.5">City *</label>
                      <input className="input" placeholder="Mumbai" {...register('venue.city')} />
                    </div>
                    <div>
                  ...
                  {watch('format') !== 'offline' && (
                    <div>
                      <label className="block text-sm font-medium mb-1.5">Online Meeting Link</label>
                      <input type="url" className="input" placeholder="https://zoom.us/..." {...register('onlineMeetingLink')} />
                    </div>
                  )}
                </div>
              )}
                    ...
                    <div key={i} className="border border-[--color-border] rounded-xl p-4 space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input className="input text-sm" placeholder="General Admission" {...register(`ticketTypes.${i}.name`)} />
                        </div>
                        <div>
              ...
              {step === 3 && (
                <div className="card p-6">
                  ...
                  <div className="bg-brand-50 dark:bg-brand-950/30 border border-brand-200 dark:border-brand-800 rounded-xl p-4 text-sm text-brand-600 dark:text-brand-400">
                    💡 Use the AI Smart Schedule Builder on the event edit page to auto-generate your agenda!
                  </div>
                </div>
              )}
